...
import { fillZero, includesIgnoreCase, randomString, toUrl } from "@/utils";
import { DefaultBodyType, http, HttpResponse, PathParams } from "msw";
import { RequiredKeysOf } from "type-fest";
import { readDB, writeDB } from "../db";
import { withAuth } from "../middleware";
import { getQueryParams, toPath } from "../utils";
...
>(toPath(toUrl(ApiRoutes.User)), ({ request }) => {
...
>(toPath(toUrl(ApiRoutes.User)), async ({ request }) => {
  const users = readUsers();
...
const updateUser = http.put<
  ...
  UserUpdate,
  ApiResponse<Nullable<number>>
>(toPath(ApiRoutes.User), async ({ request, params }) => {
  const users = readUsers();
  const user = users.find((user) => user.id === Number(params.id));
...
const approveUser = http.post<
  ...
  DefaultBodyType,
  ApiResponse<Nullable<number>>
>(toPath(ApiRoutes.ApproveUser), ({ params }) => {
  const users = readUsers();
  const user = users.find((user) => user.id === Number(params.id));
...
const createTestUsers = http.post<PathParams, UserCreate, ApiResponse<null>>(
  toPath(ApiRoutes.CreateTestUsers),
  async ({ params }) => {
