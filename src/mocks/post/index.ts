...
import { includesIgnoreCase, randomString, toUrl } from "@/utils";
...
>(toPath(toUrl(ApiRoutes.Post)), ({ request }) => {
...
>(toPath(toUrl(ApiRoutes.Post)), async ({ request }) => {
  const posts = readPosts();
...
const updatePost = http.put<
  ...
  PostUpdate,
  ApiResponse<Nullable<number>>
>(toPath(ApiRoutes.Post), async ({ request, params }) => {
  const posts = readPosts();
...
const createTestPosts = http.post<PathParams, PostCreate, ApiResponse<null>>(
  toPath(ApiRoutes.CreateTestPosts),
